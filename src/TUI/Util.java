    ...
    public static Atom getAtom(int x, int y) throws IllegalStateException
    {
        ...
        if (rayX.getExit() == rayY.getEntry())
        {
            throw new IllegalStateException(
                "Please enter side numbers with a valid intersection (example : try 37, 42)");
        }
        ...
        if (atom == null) {
            throw new IllegalStateException("Please enter side numbers with a valid intersection " +
                    "(example : try 37, 42)");
        }
        return atom;
    }
